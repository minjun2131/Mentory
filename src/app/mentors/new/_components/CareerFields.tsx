...
const CareerFields = ({ name, formReturn }: CareerFieldsProps) => {
  ...
  return (
    <li className="flex w-full gap-8 mb-5">
      ...
      />
      <YearMonthDatePicker placeholder="근무 시작 일자" name={`${name}.startDate`} formReturn={formReturn} />
      <YearMonthDatePicker placeholder="근무 종료 일자" name={`${name}.endDate`} formReturn={formReturn} />
    </li>
  );
};
